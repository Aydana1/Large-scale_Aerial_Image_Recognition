...
class RefineBlock(nn.Module):
    def __init__(self, in_channel):
        ...
        self.c3_1 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1, bias=False)   # change 512 to 256
        self.bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU(inplace=True)
        self.c3_2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False)
...
class CANet_FPN(Backbone):
    def __init__(self, out_features):
        super().__init__()
        self.backbone = CAN101(15, True)
    ...
    def forward(self, x):
        ...
        x = self.backbone.maxpool(x)  # end of stem

        l1 = self.backbone.layer1(x)   # res1
        l2 = self.backbone.layer2(l1) # res2
        l3 = self.backbone.layer3(l2)  # res3
        l4 = self.backbone.layer4(l3)  # res4
        results['p6'] = l4
        # AIDANA included


        l1 = self.backbone.rb1_1(l1)  # res1 is input to corresponding RCM
        l2 = self.backbone.rb2_1(l2)  # res2 is input to corresponding RCM
        l3 = self.backbone.rb3_1(l3)  # res3 is input to corresponding RCM
        l4 = self.backbone.rb4_1(l4)  # res4 is input to corresponding RCM 

        l4 = self.backbone.CFM(l4)    # RCM at res4 is input to CFM
        l4 = self.backbone.rb4_2(l4)  # at res4 CFM is input to RCM 

        x_fuse43 = self.backbone.fuse43(l4, l3)   #  merge RCM of res4 and AMM of res3
        x_fuse43 = self.backbone.rb3_2(x_fuse43)  # RCM gets refined by the new AMM

        # same for other blocks
        x_fuse32 = self.backbone.fuse32(x_fuse43, l2)
        x_fuse32 = self.backbone.rb2_2(x_fuse32)
        x_fuse21 = self.backbone.fuse21(x_fuse32, l1)  # merge RCM of res2 and AMM of res1
        x_fuse21 = self.backbone.rb1_2(x_fuse21) #  # RCM at res1 gets refined by its new AMM
...
class CAN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.rb4_1 = RefineBlock(2048)   #   2048  -->  256
        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.fc = nn.Linear(512 * block.expansion, num_classes)
        # only for >=res50

        # self.CFM=CFM(2048,512)
        self.CFM = CFM(256, 256)   # change 512 to 256
        self.rb4_2 = RefineBlock(256 * 5)  # change 512 to 256

        self.fuse43 = AMM(256, 256)   # change everywhere 512 to 256
        # self.post_proc43 = conv3x3_bn(512*2,512)
        self.rb3_2 = RefineBlock(256)
        self.fuse32 = AMM(256, 256)
        self.rb2_2 = RefineBlock(256)
        # self.post_proc32 = conv3x3_bn(512)
        self.fuse21 = AMM(256, 256)
        self.rb1_2 = RefineBlock(256)
        # self.post_proc21 = conv3x3_bn(512)

        self.class_conv = nn.Conv2d(256, num_classes, kernel_size=3, stride=1,
                                    padding=1, bias=True)
    ...
    # def forward(self, x):
    #     ori_size = x.size()[2:]  # get (H, W)
    #     x = self.conv1(x)
    #     x = self.bn1(x)
    #     x = self.relu(x)
    #     x = self.maxpool(x)  # end of stem

    #     l1 = self.layer1(x)   # res1
    #     l2 = self.layer2(l1) # res2
    #     l3 = self.layer3(l2)  # res3
    #     l4 = self.layer4(l3)  # res4

    #     l1 = self.rb1_1(l1)  # res1 is input to corresponding RCM
    #     l2 = self.rb2_1(l2)  # res2 is input to corresponding RCM
    #     l3 = self.rb3_1(l3)  # res3 is input to corresponding RCM
    #     l4 = self.rb4_1(l4)  # res4 is input to corresponding RCM 

    #     l4 = self.CFM(l4)    # RCM at res4 is input to CFM
    #     l4 = self.rb4_2(l4)  # at res4 CFM is input to RCM 

    #     x_fuse43 = self.fuse43(l4, l3)   #  merge RCM of res4 and AMM of res3
    #     x_fuse43 = self.rb3_2(x_fuse43)  # RCM gets refined by the new AMM

    #     # same for other blocks
    #     x_fuse32 = self.fuse32(x_fuse43, l2)
    #     x_fuse32 = self.rb2_2(x_fuse32)
    #     x_fuse21 = self.fuse21(x_fuse32, l1)  # merge RCM of res2 and AMM of res1
    #     x_fuse21 = self.rb1_2(x_fuse21) #  # RCM at res1 gets refined by its new AMM

    #     # x_fuse21=self.do(x_fuse21)
    #     x = self.class_conv(x_fuse21) 
    #     x = F.upsample(x, ori_size, mode='bilinear')  # upsample to orig size
